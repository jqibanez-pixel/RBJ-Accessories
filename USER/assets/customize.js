...
const motorLight1 = new THREE.DirectionalLight(0xffffff, 2.2);
motorLight1.position.set(8, 10, 8);
motorScene.add(motorLight1);
const motorLight2 = new THREE.DirectionalLight(0xffffff, 1.5);
motorLight2.position.set(-8, 5, 5);
motorScene.add(motorLight2);
const motorControls = new THREE.OrbitControls(motorCamera, motorRenderer.domElement);
let motorModel = null;
function loadMotor(modelName) {
    if (motorModel)
        motorScene.remove(motorModel);
...
function showPreview() {
    ...
    keyL.position.set(8, 10, 8);
    previewScene.add(keyL);
    const fillL = new THREE.DirectionalLight(0xffffff, 1.5);
    fillL.position.set(-8, 5, 5);
    previewScene.add(fillL);
