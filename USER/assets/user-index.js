...
if (coverCarousel) {
    ...
    function startAutoRotate() {
        if (rotateTimer || !coverCards.length)
            return;
    ...
    function openCoverLightbox(imageSrc, imageAlt, captionText) {
        if (!coverLightbox || !coverLightboxImage)
            return;
        stopAutoRotate();
    ...
    function closeCoverLightbox() {
        ...
        if (coverLightboxImage)
            coverLightboxImage.src = '';
        startAutoRotate();
    }
    if (dotsWrap) {
    ...
    if (prevBtn)
        prevBtn.addEventListener('click', () => updateCenterFocus(activeIndex - 1, true));
    if (nextBtn)
        nextBtn.addEventListener('click', () => updateCenterFocus(activeIndex + 1, true));
    if (coverLightboxClose)
        coverLightboxClose.addEventListener('click', closeCoverLightbox);
    if (coverLightbox) {
        coverLightbox.addEventListener('click', (e) => {
            if (e.target === coverLightbox)
                closeCoverLightbox();
        });
    }
...
branchItems.forEach((item) => {
    item.addEventListener('click', (e) => {
        if (e.target.closest('.branch-direction-btn'))
            return;
...
if (locatorMapModal) {
    locatorMapModal.addEventListener('click', (e) => {
        if (e.target === locatorMapModal)
            closeLocatorMapModal();
    });
}
function showGlobalToast(type, message) {
    if (!globalToastWrap || !message)
        return;
