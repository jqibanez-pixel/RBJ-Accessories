...
function updateThemeToggleUi(btn: HTMLButtonElement, theme: ThemeMode): void {
...
function applyTheme(mode: string | null, btn?: HTMLButtonElement): ThemeMode {
...
function insertThemeToggle(navLinks: Element): HTMLButtonElement {
...
function initNavbarThemeToggle(): void {
...
async function refreshCartCount(): Promise<void> {
  try {
    ...
    document.querySelectorAll<HTMLElement>(CART_COUNT_SELECTOR).forEach((element) => {
...
function initCartCountSync(): void {
  const badges = document.querySelectorAll<HTMLElement>(CART_COUNT_SELECTOR);
  if (!badges.length) {
    return;
  }
