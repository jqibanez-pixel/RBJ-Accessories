...
function recalculateTotal(merchandiseSubtotal) {
    ...
    if (!shippingAmountEl || !totalAmountEl)
        return;
...
(function () {
    ...
    picker.querySelectorAll('input[type="radio"][name="address_id"]').forEach(function (input) {
        input.addEventListener('change', function () {
            ...
            if (nameEl)
                nameEl.textContent = option.dataset.name || 'Receiver';
            if (contactEl)
                contactEl.textContent = option.dataset.contact || 'No contact number';
            if (addressEl)
                addressEl.textContent = option.dataset.address || 'No complete address found.';
            if (badgeEl) {
...
//# sourceMappingURL=user-buy-now.js.map